...
def track_info(tbody, fp):
	for people in tbody:
		info = people.dt.h3.text + "  "
		dds = people.findAll('dd')
		for dd in dds[1:]:
			info += dd.text.strip() + "， "
		trs = people.table.findAll('tr')
		for tr in trs:
			info += tr.find('td').text.strip()
			ais = tr.findAll('a')
			for ai in ais:
				info += ai.text + '、'
			info += '， '
...
if __name__ == "__main__":
	...
	url_prefix = 'http://www.lawyers.org.cn/searchLawFirm?page='	
	result_file = 'shanghai_firm.txt'	
	...
	while True:
		...
		info_soup = soup.findAll('div', attrs={"class":"office_list"})
		if info_soup:
