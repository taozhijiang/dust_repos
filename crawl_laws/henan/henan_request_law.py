...
def track_info(tbody, fp):
	...
	for people in trs[1:]:
		...
		fp.write("%s %s %s %s http:www.hnlawyer.org%s\n" %(tds[0].a.string, tds[1].string, tds[2].a.string, tds[3].string, tds[4].a.get('href')))
	return	
	
	
if __name__ == "__main__":
	
	url_prefix = 'http://www.hnlawyer.org/index.php/Home-index-lawyercx-is_check-1-p-'	
	result_file = 'henan_law.txt'	
	...
	while True:
		...
		info_soup = soup.find('table', attrs={"class":"tab_list"})
		if info_soup:
