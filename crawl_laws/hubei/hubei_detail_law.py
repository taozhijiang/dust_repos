...
def track_info(tbody, fp):
	...
	for i in range(9):
		if tds[i*2+1].string:
			info[tds[i*2].string.strip()] = tds[i*2+1].string.strip()
		else:
			info[tds[i*2].string.strip()] = '0'
	fp.write(repr(info)+'\n')
	return
	
if __name__ == "__main__":
	
	result_file = 'hubei_law.txt'	
	detail_file = 'hubei_detail.txt'
	...
	for line in fp:
		...
		cnt += 1
		url = line[line.index("http://www.ctlawyer.cn/index"):].strip()
		...
		info_soup = soup.find('div', attrs={"class":"lawyer_bg"})
		if info_soup:
